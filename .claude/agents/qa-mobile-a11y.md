...
**Contraste.** Depuis la version 1.5.0, le thème repose sur une échelle de douze neutres d'ardoise froide et un accent unique (`#0E6D84` le jour, `#4FC3DD` la nuit), tous construits par ratio de contraste — voir « Direction artistique » du README. Le texte principal passe largement ; les points à recalculer sont les **couleurs sourdes** (`--muted`, `--faint`), les surtitres de 11 px avec interlettrage, les compteurs de 10,5 px, et tout texte posé sur une surface floutée — surtout quand le flou n'est pas appliqué et que le repli laisse une surface plus claire. Calcule les rapports plutôt que de les estimer, dans les deux thèmes, et vise 4,5:1 pour le texte courant, 3:1 pour le texte de grande taille.
