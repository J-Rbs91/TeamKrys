...
**Flou et ombres.** Depuis la version 1.5.0, `backdrop-filter` ne subsiste qu'à **trois** endroits — barre du haut, barre d'actions, composeur — au lieu d'une quinzaine, les ombres tiennent en deux niveaux, et les deux couches fixes plein écran (aura et grain, dont une en `mix-blend-mode`) ont été supprimées. Le coût du flou se paie **pendant le défilement**, là où il se voit le plus : vérifie qu'aucune surface floutée n'est réapparue ailleurs. Vérifie qu'aucune surface floutée n'est animée ni redimensionnée en continu, et que `will-change` reste rare et ciblé : posé partout, il consomme de la mémoire au lieu d'en économiser.
